...
/**
 * 회원가입 동의 문서 (원본 /site_join_agree 텍스트, 2026-09 추출).
 * 버전 = 원본 게시 시점. 약관 개정 시 배열 수정 + 버전 갱신 → 기존 회원 재동의 필요 (v2 flow).
 */
...
const policyLines: string[] = [
  "제1조 목적",
  "",
  '본 이용약관은 “(주)엠셀”(이하 "사이트")의 서비스의 이용조건과 운영에 관한 제반 사항 규정을 목적으로 합니다.',
  "",
  "제2조 용어의 정의",
  "",
...
/** 이용약관 (필수 동의, 스크롤 박스) */
export const signupPolicyAgreement: AgreementSection[] = policyLines.reduce<
  AgreementSection[]
>((acc, line) => {
  const isHeading = /^제\d+조|^부칙/.test(line.trim());
  if (isHeading) acc.push({ heading: line.trim(), paragraphs: [] });
  else if (acc.length === 0) acc.push({ paragraphs: [line] });
  else acc[acc.length - 1].paragraphs.push(line);
  return acc;
}, []);

/** 개인정보 수집 및 이용 동의 (필수 동의, 스크롤 박스) */
export const signupPrivacyAgreement: AgreementSection[] = privacyLines.reduce<
  AgreementSection[]
>((acc, line) => {
